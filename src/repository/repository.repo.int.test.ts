...
describe('repository.repo (integration)', () => {
  it('creates a repository record', async () => {
    ...
    const created = await expectOkAsync(res);
  ...
  it('fetches a repository by full name', async () => {
    ...
    const res = repo.findByFullName('owner/repo');

    const found = await expectOkAsync(res);
  ...
  it('updates repository metadata', async () => {
    ...
    const updated = await expectOkAsync(
  ...
  it('returns null when the repository does not exist', async () => {
    ...
    const found = await expectErrAsync(repo.findByFullName('missing/repo'));
  ...
  it('finds active repositories for scanning ordered by lastCheckedAt', async () => {
    const { repo } = createTestRepo();
    await expectOkAsync(
      repo.create({ owner: 'owner', name: 'a', fullName: 'owner/a', isActive: true }),
    );
    const second = await expectOkAsync(
      repo.create({
        owner: 'owner',
        name: 'b',
        fullName: 'owner/b',
    ...
    const batch = await expectOkAsync(repo.findBatchForScanning(10));
  ...
  it('updates scan timestamps and latest tag, and falls back to DB for getLatestTag', async () => {
    ...
    const refreshed = await expectOkAsync(repo.findByFullName('owner/repo'));
